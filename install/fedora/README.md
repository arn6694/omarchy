...
The desktop retains upstream command names and configuration paths for hotkey and theme compatibility. The product name is Omadora. The upstream `SUPER + K` cheat sheet and all other key assignments are preserved.
...
The Fedora installer also installs Google Chrome Stable from Google’s official x86_64 RPM. This RPM enables Google’s repository for subsequent DNF updates. Chrome is installed separately from `base.packages` because Google’s repository is not present during the initial package availability check.

On an existing Omadora installation, run `bash ~/omarchedora/install/fedora/chrome.sh` as your normal user. It requests sudo authorization and does not rerun desktop configuration.
